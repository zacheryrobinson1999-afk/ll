...
import { createTechnician, listDaycodeUsage, readTechnicianInput, readTechnicianPatch, resetTechnicianPassword, toSafeTechnician, updateTechnician } from '../lib/adminService';
...
router.post('/admin/technicians', requireSameOrigin, async (req, res, next) => {
  ...
  catch (error) { if (duplicate(error)) { res.status(409).json({ error: 'An account with that username already exists' }); return; } if (error instanceof Error) { res.status(400).json({ error: error.message }); return; } next(error); }
});
...
router.post('/admin/technicians/:id/reset-password', requireSameOrigin, async (req, res, next) => {
  try { if (!validId(req.params.id)) { res.status(400).json({ error: 'Invalid account id' }); return; } const password = req.body && typeof req.body.password === 'string' ? req.body.password : ''; const technician = await resetTechnicianPassword(req.params.id, password); if (!technician) { res.status(404).json({ error: 'Account not found' }); return; } res.json({ technician }); }
  catch (error) { if (error instanceof Error) { res.status(400).json({ error: error.message }); return; } next(error); }
});
